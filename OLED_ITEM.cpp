...
//	OLED_SPI构造函数
OLED_SPI::OLED_SPI(u8 D0,u8 D1,u8 RES,u8 DC)
{
	this->OLED_SCL=D0;
	this->OLED_SDA=D1; 
	this->OLED_RES=RES;
...
//	x：0~15 y：0~3
//	*chr：字符串
void OLED_SPI::Print_ENString(u8 x,u8 y,const char *chr,u8 checkedState,u8 middleState)
{
	u8 i,j=0,tabNum=0;
...
void OLED_SPI::Print_FloatNum(u8 x,u8 y,u16 printNum)
{
	...
	{
		tabNum = 8-(len+3)/2;
		Print_AEN(x+0+tabNum,y,'0',0);
		Print_AEN(x+1+tabNum,y,'.',0);
		Print_AEN(x+2+tabNum,y,'0',0);
		temp = printNum % 10;
		Print_AEN(x+3+tabNum,y,temp+'0',0);
	}else if(len == 2)
	{
		tabNum = 8-(len+2)/2;
		Print_AEN(x+0+tabNum,y,'0',0);
		...
		Print_AEN(x+3+tabNum,y,temp+'0',0);
		temp = printNum % 10;
		Print_AEN(x+2+tabNum,y,temp+'0',0);
	}else
	{
		tabNum = 8-(len+1)/2;
		for(t=0;t<len+1;t++)
		{
			if(t < len+1-3)
			{
				temp=(printNum/pow(10,len-t-1))%10;
				Print_AEN(x+t+tabNum,y,temp+'0',0);
			}else if(t == len+1-3)
			{
				Print_AEN(x+t+tabNum,y,'.',0);
			...
			{
				temp=(printNum/pow(10,len-t-1+1))%10;
				Print_AEN(x+t+tabNum,y,temp+'0',0);
			}
...
/* 页面函数 */
void OLED_SPI::ENStr_Page(const char *words)
{
	Clear();
	Print_ENString(0,3,words,0,1);
}
void OLED_SPI::Comfirm_Page(const char *title,const char *words)
{
	Clear();
...
void OLED_SPI::Switch_Page(const char *title,const char *words,unsigned char state,unsigned char clr)
{
	if(clr) Clear();
